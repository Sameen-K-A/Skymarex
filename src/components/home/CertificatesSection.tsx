...
/* Old CertificateCard - designed for marquee layout */
// const CertificateCard = ({ certificate }: { certificate: ICertificate }) => {
//   return (
//     <Dialog>
//       <DialogTrigger asChild>
//         <div className="w-72 md:w-96 shrink-0 bg-muted-foreground/10 rounded-3xl p-4 cursor-pointer">
//           <div className="relative aspect-7/5 rounded-2xl overflow-hidden bg-muted-foreground/20 mb-5">
//             <Image
...
//             />
//           </div>
//           <h3 className="text-base md:text-lg font-medium mb-2 text-background">{certificate.title}</h3>
//           <p className="text-background leading-tight text-xs md:text-sm">
...
const CertificateCard = ({ certificate }: { certificate: ICertificate }) => {
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div className="w-full max-w-lg bg-muted-foreground/10 rounded-3xl p-4 md:p-5 cursor-pointer hover:bg-muted-foreground/15 transition-colors duration-300">
          <div className="relative aspect-7/5 rounded-2xl overflow-hidden bg-muted-foreground/20 mb-4 md:mb-5">
            <Image
              src={certificate.image}
              alt={certificate.title}
              fill
          ...
          <h3 className="text-base md:text-xl font-medium mb-1.5 text-background">{certificate.title}</h3>
          <p className="text-background/70 leading-snug text-xs md:text-sm tracking-tight">
            {certificate.description}
          </p>
        </div>
      </DialogTrigger>
...
export default function CertificatesSection() {
  return (
    <section className="py-12 overflow-hidden">
      ...
      {/* Marquee approach - commented out (only 2 certificates) */}
      {/* <Reveal>
      ...
      <Reveal>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6 px-4 sm:px-8">
          {certificates.map((certificate) => (
            <CertificateCard key={certificate.id} certificate={certificate} />
          ))}
        </div>
